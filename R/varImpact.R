...
corthres=0.8,dirout=NULL,outname=NULL,miss.cut=0.5)
  {
...
#              cat(" i = ",i," kk = ",kk," j = ",j,"\n")
              ...
              res = try(get.tmle.est(Yt,IA,Wtsht,deltat,Q.lib=Q.library,g.lib=g.library),silent=T)
              if(class(res)=="try-error") {errcnt=errcnt+1}
...
if(errcnt!=numcat.cont[i] & minj!=maxj) {
            ...
            res = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library),silent=T)
            if(class(res)=="try-error") {
              thetaV=c(thetaV,NA)
              varICV = c(varICV,NA)
              labV=rbind(labV,c(NA,NA))
            ...
            res2 = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library),silent=T)
...
names(outres)[1:(2*V)]=c("VarType",paste("psiV",1:V,sep=""),"AvePsi","CI95")
...
outres.cons=outres.cons[outres.cons[,"Consistent"]==TRUE,c("VarType","AvePsi","rawp"),drop=F]
...
print(xtable(outres.byV,caption="data Variable Importance Results By Estimation Sample",label="byV",digits=4),type="latex",file=paste(dirout,outname,"byV.tex",sep=""), caption.placement="top",include.rownames=T)

outres.all=outres[,c("AvePsi","CI95","rawp","BH"),drop=F]
print(xtable(outres.all, caption="data Variable Importance Results for Combined Estimates",label="allRes",digits=4),type="latex",file=paste(dirout,outname,"AllReslts.tex",sep=""),caption.placement="top",include.rownames=T)


print(xtable(outres.cons[,],caption="Subset of of Significant and ``Consistent'' Results",label="consisRes",digits=4),type="latex",file=paste(dirout,outname,"ConsistReslts.tex",sep=""),caption.placement="top",include.rownames=T)
  }
  ###########################################################################
...
if(nw > 10) {
  mydist<-as.matrix(distancematrix(t(Wt),d="cosangle",na.rm=T))
  hopach.1<-try(hopach(t(Wt),dmat=mydist,mss="mean",verbose=FALSE,K=10,kmax=3,khigh=3),silent=TRUE)
  if(class(hopach.1)=="try-error"){
    hopach.1<-try(hopach(t(Wt),dmat=mydist,mss="med",verbose=FALSE,K=10,kmax=3,khigh=3),silent=TRUE)
  }
  if(class(hopach.1)=="try-error"){
    Wtsht=Wt
    Wvsht=Wv
...
#            cat(" j = ",j,"\n")
            ...
            IA[is.na(IA)]=0
#            if(min(table(IA,Yt))>=)
            res = try(get.tmle.est(Yt,IA,Wtsht,deltat,Q.lib=Q.library,g.lib=g.library),silent=TRUE)
            if(class(res)=="try-error") {errcnt=errcnt+1}
            if(class(res)!="try-error") {
            EY1=res$theta
            if(EY1 < minEY1){
...
if(errcnt!=num.cat & minj!=maxj) {
  ...
  IA[is.na(IA)]=0
  res = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=c("SL.glmnet","SL.glm")),silent=TRUE)
  if(class(res)=="try-error") {
    thetaV=c(thetaV,NA)
    varICV = c(varICV,NA)
    labV=rbind(labV,c(NA,NA))
...
#  pp4=proc.time()
    ...
    IA[is.na(IA)]=0
    res2 = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library),silent=TRUE)
    if(class(res2)=="try-error") {
      thetaV=c(thetaV,NA)
      varICV = c(varICV,NA)
      labV=rbind(labV,c(NA,NA))
...
#              cat(" i = ",i," kk = ",kk," j = ",j,"\n")
          ...
          if(errcnt!=numcat.cont[i] & minj!=maxj) {
            IA = as.numeric(Avnew==vals[minj])
            res = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library),silent=TRUE)
            if(class(res)=="try-error") {
              thetaV=c(thetaV,NA)
              varICV = c(varICV,NA)
              labV=rbind(labV,c(NA,NA))
    ...
    if(length(out.put)==0){
      write("No VIM's could be calculated due to sample size, etc",file="AllReslts.tex")
    }
    if(length(out.put) > 0){
  lngth2=sapply(out.put,function(x) length(na.omit(x[[1]])))
    out.put=out.put[lngth2==V]
...
#    consist= (cons==1 & pval.comp > 0.05)
  ...
  names(outres)[1:(1+2*V)]=c("VarType",paste("psiV",1:V,sep=""),"AvePsi","CI95")
...
levA=levels(At)
      ...
      if(num.cat >= 2 & min(nYt,nYv) >= minYs) {
        ...
        for(j in 1:num.cat) {
          ...
          IA[is.na(IA)]=0
          #            if(min(table(IA,Yt))>=)
          res = try(get.tmle.est(Yt,IA,Wtsht,deltat,Q.lib=Q.library,g.lib=g.library))
          if(class(res)=="try-error") {errcnt=errcnt+1}
          if(class(res)!="try-error") {
            EY1=res$theta
            if(EY1 < minEY1){
        ...
        if(errcnt!=num.cat & minj!=maxj) {
          IA = as.numeric(Av==vals[minj])
          IA[is.na(IA)]=0
          res = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library))
          if(class(res)=="try-error") {
            thetaV=c(thetaV,NA)
            varICV = c(varICV,NA)
            labV=rbind(labV,c(NA,NA))
          ...
          if(class(res)!="try-error") {
            ...
            IA = as.numeric(Av==vals[maxj])
            IA[is.na(IA)]=0
            res2 = try(get.tmle.est(Yv,IA,Wvsht,deltav,Q.lib=Q.library,g.lib=g.library))
            if(class(res2)=="try-error") {
              thetaV=c(thetaV,NA)
              varICV = c(varICV,NA)
              labV=rbind(labV,c(NA,NA))
