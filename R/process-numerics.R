process_numerics =
  ...
  if (num_numeric > 0) {
    ...
    if (impute == "zero") {
      ...
      imputed = impute_median(data.num)
      data.numW = imputed$data
      impute_info = imputed$medians
    } else if (impute == "mean") {
      stop("Mean imputation not implemented yet. Please use another imputation method.")
    } else if (impute == "knn") {
      # NOTE: this also results in the data being centered and scaled.
      #
      # A row in which every column is missing has nothing to match neighbors
      # on. Such a row carries no covariate information at all, so no
      # imputation can recover one; median and zero imputation do not refuse,
      # they fill it and move on. impute_knn() does the same, setting the row
      # to 0, which is the column mean once centered and scaled, rather than
      # making knn the one method that fails on data the others accept.
      imputed = impute_knn(data.num)
      data.numW = imputed$data
      impute_info = imputed[c("center", "scale", "k")]

      if (verbose && any(imputed$all_missing)) {
        cat("Note:", sum(imputed$all_missing), "row(s) are missing every numeric",
